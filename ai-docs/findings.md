...
| AUTH-08 | Low | BookmarkController 缺 article null 檢查 → 500 而非 404。2026-09-04 複核仍在；且該 Controller 同時做跨模組編排與分頁計算（**ARCH-09**） | `BookmarkController.java:43,53,94-100` | **DONE**（2026-09-04）：收斂為共用 `resolveArticleId` helper，`findIdByUuid` 查無時拋 `BusinessException(ArticleErrorCode.ARTICLE_NOT_FOUND)`（404），`bookmark`／`unbookmark` 共用同一路徑，不會再各自漏檢查 |
...
| ARCH-09 | Medium | Controller 層做跨模組編排與分頁計算：`BookmarkController` 同時注入 3 個依賴、自行算 offset、把 `findIdByUuid` 回傳的**內部 Long 主鍵在 web 層流動**（只是碰巧沒被序列化出去），且該編排無交易邊界也無服務層測試點。null 檢查缺失即 AUTH-08 | `BookmarkController.java:34-36,71-81` | **DONE**（2026-09-04，範圍限列表路徑）：**分頁編排**（offset 計算、`filterReadableIds`、`List<Long>` 切片）下沉到新增的 `BookmarkQueryService`（`@Transactional(readOnly = true)`，有獨立單元測試 `BookmarkQueryServiceTest`），`BookmarkController` 的 `myBookmarks` 僅剩委派。**未搬移**：`bookmark`／`unbookmark` 的 UUID→PK 解析（`resolveArticleId`，見 AUTH-08 證據 `BookmarkController.java:43,53,94-100`）仍在 Controller——這兩個端點只做單筆存在性檢查，不含分頁編排，本條聚焦的是列表路徑的編排與分頁，非全面移除 Controller 對 Long PK 的接觸 | `BookmarkController.java:34-36,71-81`、`BookmarkQueryService.java:33-73` |
...
| ARCH-13 | Medium | `SeriesMapper` 直接讀 `articles` 業務表，**從基線的 4 處增為 7 處**（`architecture.md` 邊界表明列 `articles` 為業務 Data，跨模組須走 owner service；只有 reference data 可 JOIN）。`:106` 另把 `status = 'PUBLISHED'` 寫成字面字串繞過 `ArticleStatus` enum。諷刺點：**同模組的寫路徑是正確的**（走 `articleFacade.updateSeriesAssignment`），只有讀路徑破例。**性能視角**（原 PERF-34）：`findPublic` 對同一條件同時跑 COUNT 子查詢與 EXISTS 子查詢 | `SeriesMapper.java:31-44,34-35,39-40,48-49,77-78,93-94,106,115-117` | **DONE**（2026-09-04）：7 處**非同質**，逐條收斂——3 處零 series 欄位的純 articles 查詢搬回 `ArticleFacade`（`findPrevPublishedInSeries`／`findNextPublishedInSeries`／併入 `countPublishedBySeriesIds`），`:106` 的 `'PUBLISHED'` 字面字串隨之回到 `ArticleStatus` enum 所在模組；1 處（`findSeriesByArticleIds`）JOIN `articles` 只為取呼叫端已持有的 `series_id`，改 `SeriesFacade.batchGetSeriesBasicInfo` 收 `Collection<Long> seriesIds` 而非 `articleIds`，SQL 不再碰 `articles`（`ArticleQueryService.java:225-227,230-241`）；3 處真跨界的相關子查詢與 `EXISTS` 併為一次 `ArticleFacade.countPublishedBySeriesIds(seriesIds)`，回傳 `Map<seriesId, publishedCount>`。`SeriesMapper.java` 現況只剩 `series`／`users` 兩表（`SeriesMapper.java:1-115`），零 `articles`。驗收：`grep -rn "FROM articles\|JOIN articles" blog-module-series/src/main/` 零命中（原 7 處）、`grep -rn "'PUBLISHED'" blog-module-series/src/main/` 零命中。**PERF-34 隨本筆一併關閉**：`findPublic` 對同一條件的 COUNT 子查詢與 EXISTS 子查詢已合併為單次 `countPublishedBySeriesIds` 呼叫，不再雙跑。規範補完見 `architecture.md`「集合述詞（Set Predicate）」條文；新增 ArchUnit 守衛 #5（`CrossModuleBoundaryTest.java`）防退化，本 repo 先前無任何 ArchUnit 依賴（`ARCH-03`）|
